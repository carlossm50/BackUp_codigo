...
SELECT  'BANCOTUI' Sistema,
  ...
  select isnull(DE13CLI.codigo_Credito,0)cuenta_pesoa		,
    ...
    from DE13Banke 
		 full  join DE13CLI on isnull(DE13Banke.referencia,0) = cast(isnull(cast(DE13CLI.codigo_Credito as int),0) as varchar(27))




select   isnull(DE13Banke.codigo_Credito,0) Cuenta_EasyBank,
		 isnull(DE13Banke.clasif_Ini,'')	Clasificacion_Easybank,
		 isnull(DE13Banke.monto_aprobado,0)monto_apro_EasyBank,
		 isnull(DE13Banke.prov_req,0)		ProvCapReq_Easybank		 
from DE13Banke where not exists (select * from DE13CLI where isnull(DE13Banke.referencia,0) = cast(isnull(cast(DE13CLI.codigo_Credito as int),0) as varchar(27)))
go
select   isnull(DE13CLI.codigo_Credito,0) Cuenta_EasyBank,
		 isnull(DE13CLI.clasif_Ini,'')	Clasificacion_Easybank,
		 isnull(DE13CLI.monto_aprobado,0)monto_apro_EasyBank,
		 isnull(DE13CLI.prov_req,0)		ProvCapReq_Easybank		 
from DE13CLI  where not exists (select * from DE13Banke where isnull(DE13Banke.referencia,0) = cast(isnull(cast(DE13CLI.codigo_Credito as int),0) as varchar(27)))
